...
preview/
  ...
  conversations.html  ✅ Shipped — /conversations + /conversations/[conversationId] (ConversationsTable.tsx, ConversationThread.tsx, ADR 0015): dashboard-only inbox, Linear-register list (same Table pattern as bots-list.html) with bot/date/handoff-triggered filters, detail view interleaves the message transcript with tool-call log entries inline (guardrail #6 traceability)
```
