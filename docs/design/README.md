...
preview/
  ...
  conversations.html  ✅ Shipped — /conversations + /conversations/[conversationId] (ConversationsTable.tsx, ConversationThread.tsx, ADR 0015 + ADR 0016): dashboard-only inbox for non-technical review, Linear-register list (same Table pattern as bots-list.html) with bot/date/"has an issue" filters, detail view interleaves the message transcript with a plain-language summary of each tool call — raw JSON stays available behind a "Technical details" disclosure (guardrail #6 traceability, without it being the default view)
```
